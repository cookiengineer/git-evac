...
func Push(profile *structs.Profile, request *http.Request, response http.ResponseWriter) {
	...
	if request.Method == http.MethodGet {
		...
		if err == nil {
			...
			console.Log("> " + request.Method + " /api/push/" + owner + "/" + repository + ": " + http.StatusText(http.StatusOK))
			...
			console.Error("> " + request.Method + " /api/push/" + owner + "/" + repository + ": " + http.StatusText(http.StatusInternalServerError))
		...
		console.Error("> " + request.Method + " /api/push: " + http.StatusText(http.StatusMethodNotAllowed))
