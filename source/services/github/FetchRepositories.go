...
func FetchRepositories(api_url string, owner string, token string, parent_folder string) []*types.Repository {
	...
	orgas_url := fmt.Sprintf("%s/orgs/%s/repos?per_page=100&sort=updated&direction=desc", api_url, owner)
	users_url := fmt.Sprintf("%s/users/%s/repos?per_page=100&sort=updated&direction=desc", api_url, owner)
